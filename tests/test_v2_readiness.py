...
def _write_market(tmp_path, *, slug: str, clob_reconnects: int = 0, btc_reconnects: int = 0):
    ...
    pq.write_table(
        pa.Table.from_pylist([{"received_ts_ns": 1, "up_ask": 0.60, "down_ask": 0.40}]),
...
def test_root_report_counts_ready_and_blocked_markets(tmp_path):
    ...
    assert report["v2_ready_fraction"] == 0.5
